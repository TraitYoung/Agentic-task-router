...
class TaskIntent(BaseModel):
    ...
    urgency_level: int = Field(default=1, ge=1, le=5, description="紧急程度，范围只能是 1 到 5。")
    ...
    raw_input: str = Field(..., description="用户原始输入内容，必须原样保留，不允许省略。")

    @field_validator('pain_level')
    def check_meltdown_risk(cls, v):
        if v > 6:
